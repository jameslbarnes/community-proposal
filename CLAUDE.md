...
## LLM Deep Links: "Discuss with Claude / ChatGPT" Buttons

Every node and every section should have **"Discuss with Claude"** and **"Discuss with ChatGPT"** buttons. These open a new conversation in the user's Claude or ChatGPT with a pre-built prompt that:

1. Introduces the community proposal
2. Names the specific node/section the user is looking at
3. Includes the node's description and connections
4. Points to `graph-context.json` so the LLM can fetch the full graph
5. Asks the LLM to personalize the discussion based on the user's conversation history

### How It Works

The deep links use query parameter URLs:
- **Claude:** `https://claude.ai/new?q={encoded_prompt}`
- **ChatGPT:** `https://chatgpt.com/?q={encoded_prompt}`

### Implementation: `src/components/UI/DiscussButtons.tsx`
...
export function buildSectionPrompt(section: string, label: string, context: string, angle: string): string {
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          ...
          style={{
            ...
            color: '#d9773c', fontSize: 12, fontFamily: 'monospace',
        ...
        <button
          ...
          style={{
            ...
            color: '#74aa9c', fontSize: 12, fontFamily: 'monospace',
            cursor: 'pointer', transition: 'background 0.2s, border-color 0.2s',
          }}
          onMouseEnter={(e) => {
...
export function SectionDiscussButtons({ section, label, context, angle }: {
  section: string; label: string; context: string; angle: string;
}) {
  return <DiscussButtonPair prompt={buildSectionPrompt(section, label, context, angle)} />;
}
```

### Key Points

- **`COMMUNITY_NAME` and `COMMUNITY_PITCH`** must be updated in DiscussButtons.tsx when the community details are known
- **`SITE_URL`** must be updated when deployed (used for deep links back to the site and for `graph-context.json`)
- **Every node in the DetailPanel** should render `<DiscussButtons node={node} />`
- **Every section** should render `<SectionDiscussButtons>` at the bottom
- **`graph-context.json`** must be generated and kept in `public/` — this is what Claude/ChatGPT fetches to understand the full graph. Generate it with a script that flattens `graph.json` into a single context-friendly document.
- The prompt tells the LLM to check conversation history first — this personalizes the discussion to each visitor

### graph-context.json

This file lives in `public/` and is what external LLMs fetch when a visitor clicks "Discuss." It should contain:

```json
{
  "title": "Community Name",
  "description": "One paragraph describing the community",
  "url": "https://your-site.com",
  "nodes": [ /* all nodes with descriptions */ ],
  "edges": [ /* all edges */ ],
  "sections": {
    "overview": "Section 1 content summary...",
    "founder": "Section 2 content summary...",
    ...
  }
}
```

Generate this automatically from `graph.json` + section content. Add a script at `scripts/build-context.js` or have Claude generate it.
